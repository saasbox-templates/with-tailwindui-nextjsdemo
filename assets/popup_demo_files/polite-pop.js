...
`), document.body.addEventListener("click", function(n3) {
  ...
  window.PolitePop = function(options = {}) {
    ...
    if (politePopOptions.loadFullStatsDisplay) {
      let circularProgressStylesheet = document.createElement("link");
      circularProgressStylesheet.rel = "stylesheet";
      if (politePopOptions.fullStatsAssetsUseLocalUrls) {
        circularProgressStylesheet.href = "/demo/circular-progress.css";
      } else {
        circularProgressStylesheet.href = "https://cdn.politepop.com/demo/circular-progress.css?d=" + Date.now();
      }
      document.head.appendChild(circularProgressStylesheet);
